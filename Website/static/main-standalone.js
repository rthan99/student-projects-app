...
function populateProjectView(project) {
  ...
  if (project.image_urls && project.image_urls.length > 0) {
    ...
    project.image_urls.forEach((imageUrl, index) => {
      mediaHtml += `<img src="${escapeHtml(imageUrl)}" alt="${escapeHtml(project.title)} - Image ${index + 1}" onclick="showLightbox('${escapeHtml(imageUrl)}')" style="cursor: zoom-in;" />`;
    });
    mediaHtml += '</div>';
  }
  ...
  if (project.video_urls && project.video_urls.length > 0) {
    project.video_urls.forEach((videoUrl, index) => {
      if (videoUrl.includes('/storage/v1/object/public/')) {
        mediaHtml += `<video src="${escapeHtml(videoUrl)}" controls preload="metadata"></video>`;
      }
    });
  }
  ...
  if (!mediaHtml) {
    mediaHtml = `
      <div style="text-align: center; color: #64748b; padding: 3rem;">
        ...
        <p style="font-size: 1.1rem;">No media available</p>
      </div>`;
  }
...
async function populateEditForm(project) {
  try {
    // Populate all form fields (safely check if they exist)
    const setFieldValue = (id, value) => {
      const field = document.getElementById(id);
      if (field) field.value = value || '';
    };
    
    setFieldValue('edit_title', project.title);
    setFieldValue('edit_year', project.year);
    setFieldValue('edit_curator', project.curator);
    setFieldValue('edit_description', project.description);
    setFieldValue('edit_tags', project.tags);
    setFieldValue('edit_project_link', project.project_link);
    setFieldValue('edit_feedback', project.feedback);
    ...
    // Clear any previously selected new files (if they exist)
    const clearField = (id, property = 'value') => {
      const field = document.getElementById(id);
      if (field) field[property] = '';
    };
    
    clearField('edit_new_image');
    clearField('edit_new_video');
    clearField('edit-image-info', 'textContent');
    clearField('edit-video-info', 'textContent');
    
    // Set up media upload areas
    setupEditMediaUploads();
  } catch (error) {
    console.error('Error in populateEditForm:', error);
    throw error; // Re-throw to be caught by toggleEditMode
  }
}
...
async function loadComments(projectId) {
  ...
  if (!commentsList) {
    console.warn('Comments list element not found - skipping comment load');
    return;
  }
  
  if (!commentsLoading) {
    console.warn('Comments loading element not found - creating placeholder');
    commentsList.innerHTML = '<div class="comments-loading"><span>Loading comments...</span></div>';
  }
  ...
  try {
    ...
    console.log('Comments HTML rendered');
    ...
    if (commentsList) {
      commentsList.innerHTML = `<div class="comments-empty">Error loading comments: ${error.message}</div>`;
    }
  }
}
...
// Lightbox function for image zoom
